...
User.prototype.comparePassword = function (passw, cb) {
    ...
    //Add_User
    User.addUser = (firstName, lastName, city, email) => User.create({ firstName, lastName, city, email });

    //Read User By Id
    User.readUserById = (id) => User.findOne({ where: { id, isActive: true }});

    //Update User
    User.updateUser = async (id, firstName, lastName, email, city) => {
        let userData = await User.findOne({ where: { id: id, isActive: true } });
        if (userData) {
            delete userData.dataValues.password;
            let userUpdated = await User.update({ firstName, lastName, email, city }, { where: { id: id, isActive: true } });
            return userUpdated;
        }
    }
